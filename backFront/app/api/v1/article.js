...
const qnToken = require('../../../core/upload-token')
const {
  ArticleValidator,
...
/**
 * 获取七牛云token
 * new Auth(AUTH_ADMIN).m 验证是否携带token
 */
 router.get('/getQnToken',new Auth(AUTH_ADMIN).m, async (ctx) => {

    const token = await qnToken.getUploadToken();
    ctx.response.status = 200;
